...
patch_date_end = 1593302400 #release of next patch
...
used_columns = ['match_id', 'radiant_team_id', 'radiant_win', 'dire_team_id', 'radiant_score', 'dire_score', 'duration', 'radiant_gold_adv', 'radiant_xp_adv', 'picks_bans']
...
for team in teams['team_id']:
    ...
    team_result['radiant_wins'] = team_matches[(team_matches['radiant_team_id'] == team) & team_matches['radiant_win']]['match_id'].count()
    team_result['dire_win'] = team_matches[(team_matches['dire_team_id'] == team) & ~team_matches['radiant_win'].astype(bool)]['match_id'].count()
    team_result['radiant_losses'] = team_matches[(team_matches['radiant_team_id'] == team) & ~team_matches['radiant_win'].astype(bool)]['match_id'].count()
    team_result['dire_losses'] = team_matches[(team_matches['dire_team_id'] == team) & team_matches['radiant_win'].astype(bool)]['match_id'].count()
    team_result['kills'] = (team_matches[team_matches['radiant_team_id'] == team].eval('radiant_score - dire_score').sum() + team_matches[team_matches['dire_team_id'] == team].eval('dire_score - radiant_score').sum())/len(team_matches)
    team_result['duration'] = team_matches['duration'].mean()
    team_result['gold_advantage'] = ((team_matches.loc[~(team_matches['radiant_team_id'] == team), 'radiant_gold_adv'].str[-1]*-1).sum() + team_matches.loc[team_matches['radiant_team_id'] == team, 'radiant_gold_adv'].str[-1].sum())/len(team_matches)
    team_result['xp_advantage'] = ((team_matches.loc[~(team_matches['radiant_team_id'] == team), 'radiant_xp_adv'].str[-1]*-1).sum() + team_matches.loc[team_matches['radiant_team_id'] == team, 'radiant_xp_adv'].str[-1].sum())/len(team_matches)
